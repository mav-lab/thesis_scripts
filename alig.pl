...
if (exists($files->{'paired'}) && scalar(@{$files->{'paired'}})){
  ...
  $pheader .= 'RG="@RG\\tID:$ID\\tLB:$LB\\tPL:$PL\\tSM:$SM\\tPU:$PU"' . "\n\n";
  
  $pheader .= "#Alignment\n";
  $pheader .= "\$bwa mem -R \$RG -t \$nproc -a $info->{'ref_genome'} " .
              "<($catcommand \$fastq1\) <($catcommand \$fastq2) | " .
              "samblaster --discordantFile $outfolder\/$outfolder.discordant.sam --splitterFile $outfolder\/$outfolder.splitreads.sam | " .
              "\$samtools view -Shu - | " .
              "\$samtools sort -@ \$nproc -m 600000000 -T $outfolder\/tmp -o $outfolder\/$outfolder.samblaster.sorted.bam\n";
  $pheader .= "# Index\n";
  $pheader .= "cd $outfolder\n";
  $pheader .= "\$samtools index $outfolder.samblaster.sorted.bam\n";
  $pheader .= "\$samtools flagstat $outfolder.samblaster.sorted.bam >$outfolder.samblaster.flagstat\n";
